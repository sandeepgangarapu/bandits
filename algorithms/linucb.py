...
def linucb():
    ...
    X = []
    data = pd.DataFrame(X, columns=['gender', 'ethnicity', 'chosen_arm',
                                    'reward'])
    arm_list = []
    reward_list = []
    for round in range(1, num_rounds + 1):
        ...
        # data.append(pd.DataFrame({'gender': gender, 'ethnicity': ethnicity,
        #              'chosen_arm': chosen_arm,
        #              'reward': reward}, index=[0]),
        #             ignore_index=True)
        # print(data)
        # df = data[(data['gender']==gender) & (data['ethnicity']==ethnicity)]
        # df = df.groupby(['gender', 'ethnicity', 'chosen_arm'])['reward'].mean()
        # optimal_attribute = df.idxmx()
        # optimal_arm = optimal_attribute[2]
        # # regret =
        #
