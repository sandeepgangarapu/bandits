...
main_algs <- c("ab", "ucb", "thomp", "eps_greedy")
 ...
 ggplot(df %>% filter(alg == main_algs)) + geom_point(aes(x=x, y=factor(group)), shape=1, alpha=0.6) +
  ...
  ggsave("grp.png", dpi=400, height = 5, width=10, scale = 0.8)
...
```{r echo=FALSE, warning=FALSE}
ggplot(df_regret %>% filter(alg %in% c('ucb', 'thomp')), aes(x=x, y=regret)) + geom_line(aes(color=alg)) +
  xlim(0,2050) +
  geom_dl(aes(label=alg), method=list('last.points', cex=0.8)) + theme_minimal() + 
  theme(legend.position = "none") + 
  labs(x='Time Period', y = 'Regret') + 
  ggsave("reg_2.png", dpi=400, height = 5, width=10, scale = 0.8)
