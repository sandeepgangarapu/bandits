...
data <- read.csv("hyp_ite_310_t_66.csv") %>% drop_na() 

data_0 <- data %>% filter(group==0) 
data_1 <- data %>% filter(group==1)
...
final_results <- hyp_data %>% filter(!grepl("var", alg) & test_pass !=TRUE) %>% group_by(alg, test_pass) %>% summarise(cnt = n()/310)
