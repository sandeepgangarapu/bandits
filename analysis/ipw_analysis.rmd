...
true_means = c(0.9, 1, 1.1)

grp = c(0:(length(true_means)-1))
true_df <- data.frame(grp, true_means)
true_df <- true_df %>% mutate(grp=factor(grp))


data <- read.csv("athey_ite_310_t_20000_close_means.csv") %>% drop_na() %>%
  filter(mean_est != -Inf) %>% filter(mean_est != Inf) %>% filter(ite < 30)
# data2 <- read.csv("athey_ite_1000_t_20000.csv") %>% drop_na() %>%
