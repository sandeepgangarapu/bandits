...
non_agg_data <- fread("non_agg_hsn_100_2000.csv")
# remove NA's
...
setwd("G:\\My Drive\\Research\\Bandits\\code\\bandits\\analysis\\output")
agg_data <- fread("agg_hsn_100_2000.csv")
agg_data <- agg_data[complete.cases(agg_data),][mean_est != -Inf | mean_est != Inf][, mn := mean_est][, alg := ifelse(alg=="ab", "ABTesting", ifelse(alg=="thomp","THOMP", ifelse(alg=="thomp_inf", "THOMP_INF", ifelse(alg=="thomp_athey", "HADAD", NA))))]
...
mn <- ggplot(df_mse, aes(x=x, y=mse)) + geom_line(aes(color=alg)) +
  xlim(0,2500) + ylim(0,1.15) + geom_ribbon(aes(ymin=mse-(1.96*se), ymax=mse+(1.96*se), group=alg), alpha=0.4,  fill="grey70") +
  geom_dl(aes(label=alg), method=list('last.points', cex=0.8)) + theme_minimal() + 
  theme(legend.position = "none") + 
