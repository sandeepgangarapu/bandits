...
setwd("G:\\My Drive\\Research\\Bandits\\code\\bandits\\analysis\\output")
agg_data <- fread("agg_batched_bern_hyp.csv")
agg_data <- agg_data[complete.cases(agg_data),][mean_est != -Inf | mean_est != Inf][, mn := mean_est][!grepl('inf', alg) | !grepl('var', alg)]
#[alg!='thomp_inf_bern' | alg!= 'thomp_inf_bern_batched' | alg!='thomp_inf_bern_eval_aipw' | alg!= #'thomp_inf_bern_batched_eval_aipw']
agg_data_2 <- fread("thomp_inf_bern_less_xi.csv")
agg_data_2 <- agg_data_2[complete.cases(agg_data_2),][mean_est != -Inf | mean_est != Inf][, mn := mean_est]
agg_data <- rbind(agg_data, agg_data_2)
...
hyp_z <- merged_data[, zval := (mean_est.y-mean_est.x)/sqrt((((mean_est.y*arm_pull_tracker.y) + (mean_est.x*arm_pull_tracker.x))/(arm_pull_tracker.y + arm_pull_tracker.x))*(1-((mean_est.y*arm_pull_tracker.y) + (mean_est.x*arm_pull_tracker.x))/(arm_pull_tracker.y + arm_pull_tracker.x))*((1/arm_pull_tracker.y)+(1/arm_pull_tracker.x)))][,pval := pnorm(zval)][,test_pass := ifelse(pval<0.05, TRUE, FALSE)]
...
final_results <- hyp_z[!grepl("var", alg) & test_pass !=TRUE][,.(cnt = .N, perc = .N/100),.(alg, test_pass)] 
final_results <- hyp_z[!grepl("var", alg)][,.(cnt = .N, perc = .N/100),.(alg, test_pass)] 
