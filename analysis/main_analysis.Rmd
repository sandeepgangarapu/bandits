...
non_agg_data <- non_agg_data[, alg := ifelse(alg=="ab", "ABTesting", ifelse(alg=="thomp","THOMP", ifelse(alg=="thomp_inf", "THOMP_INF", NA)))]
...
setwd("G:\\My Drive\\Research\\Bandits\\code\\bandits\\analysis\\output")
agg_data <- fread("agg_analysis_default_100_20000.csv")
...
df_mse <- regret_mse[, mse := var_mse][, c('regret', 'var_mse', 'mean_mse'):=NULL][, .(mse = mean(mse), se = sd(mse)/sqrt(.N)),.(x, alg)]
