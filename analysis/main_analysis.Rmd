...
setwd("G:\\My Drive\\Research\\Bandits\\code\\bandits\\analysis\\output")
agg_data <- fread("lsn_bern_agg_1280_1000.csv")
...
setwd("G:\\My Drive\\Research\\Bandits\\code\\bandits\\analysis\\output\\xi_analysis")
options(scipen = 999)
xi = c(0.0001, 0.0005, 0.001, 0.005, 0.01, 0.05, 0.1, 0.15, 0.2, 0.5, 0.8,
          1.1, 1.5)
...
for (i in xi){
  for (j in type){
  file_name = paste("xi_analysis", i,'_',j, ".csv", sep="")
  df <- fread(file_name)
  df <- df[, xi:=i][, type:=j][, x := seq_len(.N), .(ite)]
  print(colnames(df))
  print(df$alg[1])
  print(file_name)
...
hyp_z <- merged_data[,`:=`(m1 = mean_est.x, m2 = mean_est.y, n1 = arm_pull_tracker.x, n2 = arm_pull_tracker.x)][, zval := (m2-m1)/sqrt((((m2*n2) + (m1*n1))/(n2+n1))*(1-((m2*n2) + (m1*n1))/(n2 + n1))*((1/n2)+(1/n2)))][,pval := 2*pnorm(-abs(zval))][,test_pass := ifelse(pval<0.05, TRUE, FALSE)]
...
final_results <- hyp_z[!grepl("var", alg)][,tot := .N,.(alg)][,.(cnt = .N, perc = .N/1280),.(alg, test_pass)] 
