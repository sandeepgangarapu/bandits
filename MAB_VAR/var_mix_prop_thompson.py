...
def mixed_prop_thompson(arm_means, arm_vars, num_subjects, perc_ab=0.2):
    """
    ...
    """
    print("---------------Running mix prop Thompson testing---------------")
    ...
    for subject in range(num_subjects - num_arms):
        ...
        if np.random.uniform(0, 1) < perc_ab:
            norm_prob = [var / sum(var_change_tracker) for var in
                         var_change_tracker]
            # now we pick the arm with probability proportianal to variance
            # change
            arm = np.random.choice(list(range(num_arms)), p=norm_prob)
            group_assigned.append(arm)
            outcome.append(np.random.normal(loc=arm_means[arm],
                                            scale=sqrt(arm_vars[arm])))
            # everytime we pull arm we update var_tracker and var_change_tracker
