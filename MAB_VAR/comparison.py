...
def split_outcome_lists(group, outcome):
    ...
    for i in range(len(outcome)):
        control.append([outcome[j] for j in range(i) if group[j]==0])
        trt.append([outcome[j] for j in range(i) if group[j]==1])
...
def vanilla_AB(x, y):
    ...
    # TODO: chane algo so that it takes care of variable length
