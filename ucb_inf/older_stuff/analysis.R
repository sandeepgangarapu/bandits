...
group_outcome <- read.csv("group_outcome.csv") %>% filter(type!='eps')  %>% 
  ...
  ungroup() %>% mutate(type=ifelse(type=='ab', 'AB', ifelse(type=='ucb', 'UCB', 'UCB-INF')))

regret_mse <- read.csv("regret_mse.csv") %>% 
  group_by(ite,type) %>% mutate(x=row_number()) %>% ungroup()  %>%
  ungroup() %>% mutate(type=ifelse(type=='ab', 'AB', ifelse(type=='ucb', 'UCB', 'UCB-INF')))

# Allocation of A/B testing
...
ggplot(df %>% filter(type == 'AB')) + geom_point(aes(x=x, y=factor(group)), shape=1, alpha=0.6) + facet_grid(type ~.) + theme_bw() +
  labs(x='Time Period', y = 'Group') +
  theme(axis.text.y = element_text(size = 6)) +   theme(axis.text.y = element_text(size = rel(1)))  +
  ggsave("group_ab.png", width = 10, height = 2, dpi=300, units="in")

ggplot(df %>% filter(type == 'UCB')) + geom_point(aes(x=x, y=factor(group)), shape=1, alpha=0.6) + facet_grid(type ~.) + theme_bw() +
  labs(x='Time Period', y = 'Group') +
  theme(axis.text.y = element_text(size = 6)) +   theme(axis.text.y = element_text(size = rel(1)))  +
  ggsave("group_ucb.png", width = 8, height = 2, dpi=300, units="in")


ggplot(df %>% filter(type == 'UCB-INF')) + geom_point(aes(x=x, y=factor(group)), shape=1, alpha=0.6) + facet_grid(type ~.) + theme_bw() +
  labs(x='Time Period', y = 'Group') +
  theme(axis.text.y = element_text(size = 6)) +   theme(axis.text.y = element_text(size = rel(1)))  +
